...
export default function YearOfBirthChart({ birthYearStats, total }: YearOfBirthChartProps) {
  const chartData = useMemo(() => {
    return Object.entries(birthYearStats)
      .map(([year, count]) => ({
        year: parseInt(year),
        count,
      ...
      .slice(-20) // Show last 20 years
  }, [birthYearStats, total])

  if (chartData.length === 0) {
    return (
      <div className="bg-white rounded-[20px] shadow-drop p-6">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Phân bố theo năm sinh</h3>
